      ...
      d_num_threads(num_threads)
{
    ...
    d_logger->debug("filter len = {}", d_taps.size());
...
{
    d_logger->info("Stopped with {} bursts remaining in queue", d_bursts.size());
    d_logger->info("Emitted {} bursts", ++d_max_id);
    ...
    for (size_t i = 0; i < work_buffer.new_burst_tags.size(); i++) {
        ...
        if (d_lower_border < relative_frequency && relative_frequency <= d_upper_border) {
            ...
            if (d_bursts.count(id)) {
                d_logger->error("tagged_burst_to_pdu saw a repeated burst id "
                                "({}), discarding...", id);
            ...
            if (id == 1) {
                d_logger->info("start = {} {}", start_offset, tag.offset);
            }
...
void tagged_burst_to_pdu_impl::publish_and_remove_old_bursts(const buffer& work_buffer)
{
    for (tag_t tag : work_buffer.gone_burst_tags) {
        ...
        if (d_bursts.count(id)) {
            ...
            if (d_debug) {
                d_logger->info("gone burst: {} {}", id, burst.len);
            }
            ...
            if (burst.len >= d_min_burst_size) {
                if (id == 1) {
                    d_logger->info("id {}: len = {} {}", id, burst.len, tag.offset);
                }
                publish_burst(burst);
            }
    ...
    for (std::pair<const uint64_t, burst_data>& burstpair : d_bursts) {
        ...
        if (burst.len >= d_max_burst_size) {
            ...
            if (d_debug) {
                d_logger->info("gone (long) burst: {} {}", id, burst.len);
            }
            if (burst.len >= d_min_burst_size) {
                publish_burst(burst);
            }
    ...
    while (true) {
        if (d_work_queue.pop(work_buffer)) {
            ...
            if (d_bursts.size() > max_cbursts) {
                d_logger->info("New max bursts from {} to {}",
